...
You and every agent you spawn are bound by
[agent-authority](../../rules/agent-authority.md). For you the sharp edges are: **never
file an issue**, never merge, never close anything, and never touch an issue or PR that
didn't opt in.
...
- **Max 2 developers at once.** Over the cap, leave the issue unclaimed; the next tick
  picks it up.
- **Only one developer may run the full test suite at a time.** Mail and object storage
  are shared across worktrees — `layers/email-mailgun/server/utils/email.ts` hardcodes
  `localhost:1025` — so two suites running together read each other's messages and
  objects, and the failures look like flakes rather than interference. Concurrent
  developers are fine while they're writing code; stagger them if they'll be running
  `test`/`test:e2e` at the same time, or let CI be the place the full suite runs.
...
saying what's stuck and what you tried — ending with the footer from
[github-comments](../../rules/github-comments.md). Then leave it alone. Escalating to a
human is a success condition, not a failure.

Report the same thing back to whoever started you. A blocked item nobody is told about is
a dropped item.

## First run

Do a dry pass before turning this loose: run the tick with delegation disabled and print
what you *would* have claimed and spawned. Confirm the list is what you expect — that no
human's issue is in it, and no already-owned PR is in it — then enable delegation.

<!-- TODO: fill in — whether upstream/corsacca issues are in scope, alerting on blocked items -->
