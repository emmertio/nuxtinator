---
name: worktree
description: Set up and tear down the git worktrees used for parallel feature work — give each its own branch, ports, env, and databases, and retire worktrees whose PR has merged. Use when starting any new unit of work, and after a PR merges to clean up.
---
...
checkouts sharing either corrupt each other's results in ways that look like code bugs.

## First: are you already in a worktree?

**Check before creating anything** — `git rev-parse --show-toplevel` and `pwd`.

Subagents launched with worktree isolation are **already placed in one** by the harness,
at `.claude/worktrees/agent-<id>/` on a branch it created, and git commands aimed at the
parent checkout are blocked. In that case creating a worktree is impossible and
unnecessary:

- **Skip to step 3.** Ports, env, and databases still need doing — the harness gives you a
  checkout, not a runnable app.
- To base your work on a branch other than the one you were given, `git fetch origin` and
  `git reset --hard origin/<branch>` **inside your own worktree**. Don't try to re-create
  it elsewhere.
- Don't sweep for retired worktrees; you can't see or safely remove the others.

The rest of this section is for the parent checkout — a human, or an agent that was not
isolated. Those worktrees live at `worktrees/<name>/` (gitignored).
...
3. **Derive the port slot — don't search for a free one.** Slot `N = (issue number % 8) + 1`,
   ...
   **Deriving beats scanning.** "Lowest slot not currently in use" looks fine and races:
   ...
   | | Parent | Slot 1 | Slot 2 | Isolated? |
   | --- | --- | --- | --- | --- |
   | Dev server (`NUXT_PORT`) | 2080 | 2180 | 2280 | yes |
   | Playwright (`TEST_BASE_URL`) | 2090 | 2190 | 2290 | yes |
   | Test database | `go_saas_test` | `go_saas_test_1` | `go_saas_test_2` | yes |
   | Dev database | `apps` | `apps_1` | `apps_2` | yes |
   | Mailpit (SMTP 1025 / API 8025) | shared | shared | shared | **no** |
   | S3 / MinIO (9000) | shared | shared | shared | **no** |

   Formula: dev `2080 + N*100`, e2e `2090 + N*100`. A block of 100 per slot leaves room.

   **Mail and object storage are not slottable, so mail and upload tests cannot run in
   two worktrees at once.** `layers/email-mailgun/server/utils/email.ts` hardcodes
   `localhost:1025`, and the S3 endpoint comes from env the app reads globally. Two
   suites running together will read each other's messages and objects, and the failures
   look like flakes.

   Until that's fixed: **run the full suite in one worktree at a time**, or let CI be the
   place the full suite runs — it gets a clean Mailpit and MinIO per job. Lint, typecheck,
   and non-mail unit tests are safe to run concurrently.
...
5. **Give the worktree its own databases.** Both matter:
   ...
   The script defaults to Postgres on **5432**; a Docker-hosted Postgres here commonly
   listens on **5434**, and a password-protected superuser needs `PGPASSWORD`. Both are
   env overrides — `TEST_DB_PORT`, `TEST_DB_SUPERUSER`, `PGPASSWORD` — so check where
   Postgres actually is before concluding the script is broken.

6. **Start object storage** — `./scripts/setup-test-s3.sh`. Uploads tests need it, and it
   is shared across worktrees (see the table above).

7. **`bun install` in the worktree** — `node_modules` is per-checkout, and this is a bun
